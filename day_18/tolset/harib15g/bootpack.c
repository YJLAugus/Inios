...
void HariMain(void)
{
	...
	*((int *) (task_cons->tss.esp + 8)) = memtotal;
	...
	make_window8(buf_win, 144, 52, "task_a", 1);//窗口标题
		...
		if (fifo32_status(&fifo) == 0) {
			...
			if (256 <= i && i <= 511) { /* 键盘数据 */
				...
				if (mouse_decode(&mdec, i - 512) != 0) {
					...
					sheet_slide(sht_mouse, mx, my);/* 里面包含sheet_refresh */
					if ((mdec.btn & 0x01) != 0) {
						/* 按下左键，移动sht_win */
				...
				if (i != 0) {
					timer_init(timer, &fifo, 0); /* 下次置为0 */
					if (cursor_c >= 0) {
						cursor_c = COL8_000000;
					}
				} else {
					timer_init(timer, &fifo, 1); /* 下次置为1 */
					if (cursor_c >= 0) {
						cursor_c = COL8_FFFFFF;
					}
				}
...
void console_task(struct SHEET *sheet, unsigned int memtotal)
{
	...
	for (;;) {
		...
		if (fifo32_status(&task->fifo) == 0) {
			...
			if (i <= 1) { /* 光标用定时器 */
				if (i != 0) {
					timer_init(timer, &task->fifo, 0); /* 设置为0 */
					if (cursor_c >= 0) {
						cursor_c = COL8_FFFFFF;
					}
				} else {
					timer_init(timer, &task->fifo, 1); /* 设置为1 */
					if (cursor_c >= 0) {
						cursor_c = COL8_000000;
					}
				}
